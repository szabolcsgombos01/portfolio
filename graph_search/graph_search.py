...
class LabyrinthBoard(Board):
    ...
    def load(self, path: str) -> None:
        ...
        for row in self.board:
            assert len(row) == len(
                self.board[0]), "The board should be rectangular!"
...
class CatToDoorProblem(SearchProblem):
    ...
    def __init__(self, game_logic: GameLogic):
        ...
        self.door_positions = game_logic.collect_positions(
            LabyrinthFields.Door)
        self.walls = [
...
def manhattan(problem: SearchProblem):
    ...
    def heuristics(state: State):
        ...
        for target_position in target_positions:
            vertical_distance = abs(position.row - target_position.row)
            horizontal_distance = abs(position.col - target_position.col)
...
class EatDonutsProblem(SearchProblem):
    ...
    def target_positions(self, state: EatDonutsState) -> tuple[Position, ...]:
        if state.donuts:  # If there are donuts left, those are the targets
            return state.donuts
        else:  # Otherwise, it's the door(s)
            return self.door_positions

    def is_goal_state(self, state: EatDonutsState) -> bool:
        # If all the donuts are eaten and we're at the door, it's the goal state
    ...
    def next_states(self, state: EatDonutsState) -> set[EatDonutsState]:
        ...
        for action in Actions:
            ...
            if (
                ...
                donuts = state.donuts
                if new_position in donuts:
                    # If the new position has a donut, remove it from the donuts set
                    donuts = tuple(d for d in donuts if d != new_position)
...
while True:  # Event Loop
    ...
    window.Element("heuristics").Update(
        disabled=values["algorithm"] not in {
            "Look forward search", "A* search"}
    )
